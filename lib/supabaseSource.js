...
async function obtenerEstadisticasRutas(desde, hasta) {
  const data = await selectAll('rutas_paradas', 'tipo,vehiculo,conductor,fecha,incidencia', function (query) {
    if (desde) query = query.gte('fecha', desde);
    if (hasta) query = query.lte('fecha', hasta);
    return query;
  });
  ...
  data.forEach(function (r) {
    ...
    if (!r.conductor && !r.vehiculo) sinAsignar++;

    if (r.conductor) {
      if (!porConductor[r.conductor]) porConductor[r.conductor] = { nombre: r.conductor, entregas: 0, recogidas: 0, total: 0 };
      porConductor[r.conductor].total++;
      if (r.tipo === 'ENTREGA') porConductor[r.conductor].entregas++;
      else if (r.tipo === 'RECOGIDA') porConductor[r.conductor].recogidas++;
    }
    if (r.vehiculo) {
      if (!porVehiculo[r.vehiculo]) porVehiculo[r.vehiculo] = { nombre: r.vehiculo, entregas: 0, recogidas: 0, total: 0 };
      porVehiculo[r.vehiculo].total++;
      if (r.tipo === 'ENTREGA') porVehiculo[r.vehiculo].entregas++;
      else if (r.tipo === 'RECOGIDA') porVehiculo[r.vehiculo].recogidas++;
    }
  });
...
module.exports = { llamarOrumCentralSupabase, obtenerMaterialDeProyecto, obtenerDetalleProyecto, obtenerConductores, obtenerEstadoVehiculos, obtenerEstadisticasRutas, ACCIONES, supabase };
