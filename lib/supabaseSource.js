...
async function obtenerEstadisticasRutas(desde, hasta, filtroNave) {
  ...
  function esNave_(loc) {
    var l = String(loc || '').toUpperCase();
    return l.indexOf('NAVE MARBELLA') !== -1 || l.indexOf('NAVE MONDA') !== -1;
  }

  var totalInterno = paradasDataFull.filter(function (r) { return esNave_(r.localizacion); }).length;
  var paradasData = paradasDataFull.filter(function (r) {
    var interno = esNave_(r.localizacion);
    if (filtroNave === 'excluir') return !interno;
    if (filtroNave === 'solo') return interno;
    return true;
  });
