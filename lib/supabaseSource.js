...
async function obtenerDetalleProyecto(proyectoId) {
  const [{ data: equipmentData, error: errEq }, { data: proyectoData, error: errProy }] = await Promise.all([
    ...
    supabase.from('proyectos').select('transport_price,crew_price,insurance_price,other_price,sale_price').eq('id', proyectoId).maybeSingle()
  ]);
  ...
  const servicios = proyectoData
    ...
    : [];
  return { material: material, servicios: servicios };
}
...
module.exports = { llamarOrumCentralSupabase, obtenerMaterialDeProyecto, obtenerDetalleProyecto, obtenerConductores, obtenerEstadoVehiculos, ACCIONES, supabase };
