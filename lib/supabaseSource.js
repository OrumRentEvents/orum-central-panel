...
// filtroNave: undefined/'todos' -> incluye todo; 'excluir' -> quita Nave
// Marbella/Monda (movimientos internos de almacén, sin cliente/ingreso
// real); 'solo' -> solo esos, para verlos aparte y no confundir las cifras
// con entregas/recogidas de cliente reales (por eso entregas y recogidas
// no cuadran entre sí: las de nave son casi siempre solo ENTREGA).
async function obtenerEstadisticasRutas(desde, hasta, filtroNave) {
  const [paradasDataFull, conductoresData] = await Promise.all([
    selectAll('rutas_paradas', 'tipo,vehiculo,conductor,fecha,vuelta,incidencia,localizacion', function (query) {
      if (desde) query = query.gte('fecha', desde);
      if (hasta) query = query.lte('fecha', hasta);
      return query;
    }),
  ...
  function naveDe_(loc) {
    var l = String(loc || '').toUpperCase();
    return { marbella: l.indexOf('NAVE MARBELLA') !== -1, monda: l.indexOf('NAVE MONDA') !== -1 };
  }

  var totalInterno = 0, totalMarbella = 0, totalMonda = 0;
  paradasDataFull.forEach(function (r) {
    var n = naveDe_(r.localizacion);
    if (n.marbella || n.monda) totalInterno++;
    if (n.marbella) totalMarbella++;
    if (n.monda) totalMonda++;
  });

  // filtroNave: 'todos' (nada filtrado) | 'excluir' (quita Marbella y Monda)
  // | lista separada por comas con 'marbella' y/o 'monda' (solo esas naves,
  // seleccionables por separado o juntas).
  var navesSeleccionadas = (filtroNave && filtroNave !== 'todos' && filtroNave !== 'excluir') ? filtroNave.split(',') : [];
  var paradasData = paradasDataFull.filter(function (r) {
    var n = naveDe_(r.localizacion);
    if (filtroNave === 'excluir') return !n.marbella && !n.monda;
    if (navesSeleccionadas.length > 0) {
      return (navesSeleccionadas.indexOf('marbella') !== -1 && n.marbella) || (navesSeleccionadas.indexOf('monda') !== -1 && n.monda);
    }
    return true;
  });
  ...
  return {
    ...
    total_marbella: totalMarbella,
    total_monda: totalMonda,
    filtro_nave: filtroNave || 'todos',
